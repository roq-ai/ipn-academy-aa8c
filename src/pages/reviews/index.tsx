...
function ReviewListPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<ReviewInterface[]>(
    ...
    () =>
      getReviews({
        relations: ['user_review_user_idTouser', 'user_review_trainer_idTouser'],
      }),
  );
              ...
              <Thead>
                <Tr>
                  ...
                  {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                    <Th>user_review_user_idTouser</Th>
                  )}
                  {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                    <Th>user_review_trainer_idTouser</Th>
                  )}
              ...
              <Tbody>
                {data?.map((record) => (
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>
                        <Link as={NextLink} href={`/users/view/${record.user_review_user_idTouser?.id}`}>
                    ...
                    {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>
                        <Link as={NextLink} href={`/users/view/${record.user_review_trainer_idTouser?.id}`}>
                          {record.user_review_trainer_idTouser?.email}
                        </Link>
                      </Td>
                    )}
